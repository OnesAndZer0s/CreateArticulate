package org.onesandzer0s.create_articulate.gui.engine
...
class EngineScreen(handler: EngineScreenMenu, playerInventory: Inventory, text: Component) :
    ...
    override fun renderBg(guiGraphics: GuiGraphics, partialTicks: Float, mouseX: Int, mouseY: Int) {
        ...
        val (containerX, containerY) =
                if (menu.heatLevel > 1) Pair(HEATED_CONTAINER_X, HEATED_CONTAINER_Y)
                else Pair(CONTAINER_X, CONTAINER_Y)
        ...
        if (menu.fuelLeft != 0) {
            val t =
                    COAL_MULTI_MAX -
                            ((menu.fuelLeft.toFloat() / menu.fuelTotal.toFloat()) * COAL_MULTI_MAX)
        ...
        val (glassX, glassY) =
                if (menu.heatLevel > 3) Pair(HEATED_GLASS_X, HEATED_GLASS_Y)
                else Pair(GLASS_X, GLASS_Y)
    ...
    companion object { // TEXTURE DATA
        internal val TEXTURE =
                ResourceLocation(CreateArticulateMod.MOD_ID, "textures/gui/engine.png")
