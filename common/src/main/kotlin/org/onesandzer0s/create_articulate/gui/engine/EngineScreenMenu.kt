...
import org.onesandzer0s.create_articulate.util.inventorySlots

class EngineScreenMenu(syncId: Int, playerInv: Inventory, val blockEntity: EngineBlockEntity?) :
        AbstractContainerMenu(CreateArticulateScreens.ENGINE.get(), syncId) {
    ...
    override fun quickMoveStack(player: Player, index: Int): ItemStack {
        ...
        if (slot != null &&
                        slot.hasItem() &&
                        (AbstractFurnaceBlockEntity.isFuel(slot.item) || isBucket(slot.item))
        ) {
            if (index != 0) {
