...
class CreateArticulateShipControl : ShipForcesInducer, ServerTickListener {
    ...
    val canDisassemble
        get() =
                ship != null &&
                        ...
                        positionUntilAligned.distanceSquared(
                                this.ship!!.transform.positionInWorld
                        ) < 4.0
    val aligningTo: Direction
        get() = Direction.from2DDataValue(alignTarget)
    var consumed = 0f
        private set

    private var wasCruisePressed = false
    @JsonProperty("cruise") var isCruising = false
    ...
    override fun applyForces(physShip: PhysShip) {
        ...
        physShip.doFluidDrag = CreateArticulateConfig.SERVER.doFluidDrag
        ...
        val buoyantFactorPerFloater =
                min(
                        CreateArticulateConfig.SERVER.floaterBuoyantFactorPerKg / 15 / mass,
                        CreateArticulateConfig.SERVER.maxFloaterBuoyantFactor
                )

        physShip.buoyantFactor = 1.0 + floaters * buoyantFactorPerFloater
        // Revisiting create_articulate control code.
        ...
        if ((aligning) && abs(angleUntilAligned) > ALIGN_THRESHOLD) {
            ...
            val idealOmega =
                    Vector3d(invRotationAxisAngle.x, invRotationAxisAngle.y, invRotationAxisAngle.z)
                            .mul(-angleUntilAligned)
                            .mul(CreateArticulateConfig.SERVER.stabilizationSpeed)
        ...
        if (player != null) {
            ...
            if (!wasCruisePressed && player.cruise) {
                ...
                showCruiseStatus()
                            ...
                            isCruising &&
                            (player.leftImpulse != 0.0f ||
                                    player.sprintOn ||
                                    player.upImpulse != 0.0f ||
                                    player.forwardImpulse != 0.0f) &&
        ...
        controlData?.let { control ->
            ...
            val largestDistance =
                    run {
                            ...
                            .coerceIn(0.5, CreateArticulateConfig.SERVER.maxSizeForTurnSpeedPenalty)

            val maxLinearAcceleration = CreateArticulateConfig.SERVER.turnAcceleration
            val maxLinearSpeed = CreateArticulateConfig.SERVER.turnSpeed + extraForceAngular
            ...
            val normalizedAlphaYMultiplier =
                    if (isBelowMaxTurnSpeed && control.leftImpulse != 0.0f)
                            control.leftImpulse.toDouble()
            ...
            val angularImpulse =
                    stw.transformDirection(
                            moiTensor.transform(wts.transformDirection(Vector3d(alpha)))
                    )
            ...
            physShip.poseVel.rot.transform(
                    moiTensor.transform(physShip.poseVel.rot.transformInverse(rotationVector))
            )
            ...
            val velOrthogonalToPlayerUp =
                    vel.sub(
                            playerUpDirection.mul(playerUpDirection.dot(vel), Vector3d()),
                            Vector3d()
                    )

            // This is the speed that the ship is always allowed to go out, without engines
            val baseForwardVel =
                    Vector3d(forwardVector).mul(CreateArticulateConfig.SERVER.baseSpeed)
            val baseForwardForce =
                    Vector3d(baseForwardVel).sub(velOrthogonalToPlayerUp).mul(mass * 10)

            // This is the maximum speed we want to go in any scenario (when not sprinting)
            val idealForwardVel =
                    Vector3d(forwardVector).mul(CreateArticulateConfig.SERVER.maxCasualSpeed)
            val idealForwardForce =
                    Vector3d(idealForwardVel).sub(velOrthogonalToPlayerUp).mul(mass * 10)
            ...
            if (extraForceLinear != 0.0) {
                actualExtraForce.fma(
                        min(extraForceLinear / extraForceNeeded.length(), 1.0),
                        extraForceNeeded
                )
            }
            ...
            if (control.upImpulse != 0.0f) {
                idealUpwardVel =
                        Vector3d(0.0, 1.0, 0.0)
                                ...
                                .mul(
                                        CreateArticulateConfig.SERVER.baseImpulseElevationRate +
                                                // Smoothing for how the elevation scales as you
                                                // approaches the balloonElevationMaxSpeed
                                                smoothing(
                                                        2.0,
                                                        CreateArticulateConfig.SERVER
                                                                .balloonElevationMaxSpeed,
                                                        balloonForceProvided / mass
                                                )
                                )
            }
        }
        ...
        val idealUpwardForce =
                Vector3d(
                                ...
                                idealUpwardVel.y() -
                                        vel.y() -
                                        (GRAVITY /
                                                CreateArticulateConfig.SERVER.elevationSnappiness),
                                0.0
                        )
                        .mul(mass * CreateArticulateConfig.SERVER.elevationSnappiness)

        val actualUpwardForce =
                Vector3d(0.0, min(balloonForceProvided, max(idealUpwardForce.y(), 0.0)), 0.0)
    ...
    private fun showCruiseStatus() {
        val cruiseKey =
                if (isCruising) "hud.create_articulate.start_cruising"
                else "hud.create_articulate.stop_cruising"
    ...
    private fun deleteIfEmpty() {
        if (helms <= 0 && floaters <= 0 && anchors <= 0 && balloons <= 0) {
            ship?.saveAttachment<CreateArticulateShipControl>(null)
        }
    }

    /** f(x) = max - smoothing / (x + (smoothing / max)) */
    private fun smoothing(smoothing: Double, max: Double, x: Double): Double =
            max - smoothing / (x + (smoothing / max))

    companion object {
        fun getOrCreate(ship: ServerShip): CreateArticulateShipControl {
            return ship.getAttachment<CreateArticulateShipControl>()
                    ?: CreateArticulateShipControl().also { ship.saveAttachment(it) }
        }

        private const val ALIGN_THRESHOLD = 0.01
        private const val DISASSEMBLE_THRESHOLD = 0.02
        private val forcePerBalloon
            get() = CreateArticulateConfig.SERVER.massPerBalloon * -GRAVITY
