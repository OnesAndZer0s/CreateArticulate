package org.onesandzer0s.create_articulate.util
...
object ShipAssembler {
    ...
    private fun roundToNearestMultipleOf(number: Double, multiple: Double) =
            multiple * round(number / multiple)

    // modified from
    // https://gamedev.stackexchange.com/questions/83601/from-3d-rotation-snap-to-nearest-90-directions
    ...
    fun unfillShip(
        ...
        val rotation: Rotation =
                when (direction) {
                    Direction.SOUTH ->
                            Rotation.NONE // Bug in Direction.from2DDataValue() can return
                    // south/north as opposite
        ...
        level.server.executeIf(
            ...
            // Once all the chunk updates are sent to players, we can tell them to restart chunk
            // updates
            level.players().forEach { player ->
    ...
    private fun bfs(
        ...
        while (!stack.isEmpty) {
            ...
            if ((CreateArticulateConfig.SERVER.maxShipBlocks > 0) and
                            (blocks.size > CreateArticulateConfig.SERVER.maxShipBlocks)
            ) {
                logger.info("Stopped ship assembly due too many blocks")
                return false
            }
        }
        if (CreateArticulateConfig.SERVER.maxShipBlocks > 0) {
            logger.info(
                    "Assembled ship with ${blocks.size} blocks, out of ${CreateArticulateConfig.SERVER.maxShipBlocks} allowed"
            )
        }
        return true
    }
    ...
    private fun directions(center: BlockPos, lambda: (BlockPos) -> Unit) {
        if (!CreateArticulateConfig.SERVER.diagonals)
